...
# ----------------------------------------------------------------------------- parameters
N_MODULES   = 27             # overridden by --modules N (the first N of the shipped 27)
MODEL_NAME  = "cubot_shipped"
...
def roll_word():
    ...
    assert len(ROLL_WORD) == 26 and set(ROLL_WORD) <= set("0123")
    # A shorter chain (--modules N) is the first N modules of the shipped one: the same mounts, truncated.
    return [int(d) for d in ROLL_WORD[: N_MODULES - 1]]
...
def build_urdf(roll, props):
    out = ['<?xml version="1.0"?>',
           ...
           f"     {N_MODULES} x 80 mm cubes, <111> hinge, 8 mm equatorial chamfers + CAD neck profile, {GAP*1000:.1f} mm gap (pitch {PITCH*1000:.1f} mm),",
           f"     shipped roll word {''.join(map(str, roll))} (right-hand about +X, toward module {N_MODULES}"
           + ("" if N_MODULES == 27 else f"; the first {N_MODULES} modules of the 27-module shipped chain") + "). Units: m, rad, kg.",
...
def build_mjcf(roll, props):
    ...
    return f"""<!-- Generated by build_cubot_urdf.py; edit that script, not this file.
     Same chain as {MODEL_NAME}.urdf, with a free base so it lies loose on the table.
...
SCENE = """<mujoco model="{MODEL_NAME} scene">
  <!-- Load THIS file. It adds the floor, lights and camera around the chain in {MODEL_NAME}.xml. -->
  <include file="{MODEL_NAME}.xml"/>

  <statistic center="{CENTER_X:.2f} 0 0.05" extent="{EXTENT:.1f}"/>
  <visual>
...
def main():
    import argparse
    global N_MODULES, MODEL_NAME
    ap = argparse.ArgumentParser(description="Build the CuBot chain as URDF + MJCF.")
    ap.add_argument("--modules", type=int, default=27,
                    help="chain length: the first N modules of the 27-module shipped chain (default 27)")
    args = ap.parse_args()
    if not 2 <= args.modules <= 27:
        raise SystemExit("--modules must be in 2..27")
    N_MODULES = args.modules
    MODEL_NAME = "cubot_shipped" if N_MODULES == 27 else f"cubot_{N_MODULES}"
    scene_name = "scene.xml" if N_MODULES == 27 else f"scene_{N_MODULES}.xml"
    roll = roll_word()
    ...
    span = (N_MODULES - 1) * PITCH
    scene = SCENE.replace("{MODEL_NAME} scene", ("cubot shipped" if N_MODULES == 27 else MODEL_NAME) + " scene").replace("{MODEL_NAME}", MODEL_NAME).replace("{CENTER_X:.2f}", f"{span / 2:.2f}") \
                 .replace("{EXTENT:.1f}", f"{max(1.0, span + 0.3):.1f}")
    with open(os.path.join(HERE, f"{MODEL_NAME}.urdf"), "w") as f:
        f.write(build_urdf(roll, props))
    with open(os.path.join(HERE, f"{MODEL_NAME}.xml"), "w") as f:
        f.write(build_mjcf(roll, props))
    with open(os.path.join(HERE, scene_name), "w") as f:
        f.write(scene)
    print(f"wrote {MODEL_NAME}.urdf, {MODEL_NAME}.xml, {scene_name}, meshes/still.stl, meshes/moving.stl "
          f"({N_MODULES} modules, roll {''.join(map(str, roll))}, pitch {PITCH*1000:.1f} mm, span {span*1000:.1f} mm)")
